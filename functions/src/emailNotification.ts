...
function shouldSendEmailForTipo(tipo: TipoNotificacao, config: ConfigEmail): boolean {
...
function buildEmailHtml(titulo: string, mensagem: string, link?: string, baseUrl?: string): string {
 ...
 */
async function getConfig(userId: string): Promise<ConfigEmail | null> {
  try {
...
export const onNotificacaoCriada = onDocumentCreated(
  ...
  async (event) => {
    ...
    const tipo = data?.tipo as TipoNotificacao | undefined;
    const titulo = (data?.titulo as string) ?? "Notificação";
    ...
    const baseUrl = (process.env.APP_URL as string)?.trim() || "";
    try {
  ...
  async (request: CallableRequest) => {
    ...
    if (!resend) {
      throw new HttpsError(
        "failed-precondition",
        "Envio de e-mail não está configurado (RESEND_API_KEY). Entre em contato com o administrador."
      );
    }
    ...
    if (!to) {
      throw new HttpsError(
        "failed-precondition",
        "Sua conta não possui e-mail. Não é possível enviar o e-mail de teste."
      );
    }
